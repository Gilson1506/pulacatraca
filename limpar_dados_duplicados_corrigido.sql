-- ===================================================================
-- SCRIPT PARA LIMPAR DADOS DUPLICADOS - CORRIGIDO
-- ===================================================================
...
WITH registros_numerados AS (
    SELECT 
        ...
        ticket_id,
        created_at,
        ROW_NUMBER() OVER (PARTITION BY qr_code ORDER BY created_at DESC) as ranking
    FROM ticket_users
)
SELECT 
...
FROM registros_numerados
WHERE ranking > 1
ORDER BY qr_code, created_at;

-- ===================================
-- 4. SCRIPT DE LIMPEZA CORRIGIDO (DESCOMENTE PARA EXECUTAR)
-- ===================================
...
-- Remover duplicados usando Window Function (método correto)
WITH registros_a_manter AS (
    SELECT id
    FROM (
        SELECT 
            id,
            ROW_NUMBER() OVER (PARTITION BY qr_code ORDER BY created_at DESC) as rn
        FROM ticket_users
    ) ranked
    WHERE rn = 1
)
DELETE FROM ticket_users 
WHERE id NOT IN (SELECT id FROM registros_a_manter);
...
-- Remover duplicados apenas deste QR (método corrigido)
WITH registro_a_manter AS (
    SELECT id 
    FROM ticket_users 
    WHERE qr_code = 'PLKTK909538'
    ORDER BY created_at DESC 
...
DELETE FROM ticket_users 
WHERE qr_code = 'PLKTK909538'
AND id NOT IN (SELECT id FROM registro_a_manter);

-- Verificar resultado específico
SELECT 
    'RESULTADO_LIMPEZA_ESPECIFICA' as status,
...
-- ATENÇÃO: Só funciona APÓS limpar os duplicados existentes
CREATE UNIQUE INDEX idx_ticket_users_qr_code_unique 
ON ticket_users (qr_code);
*/

-- ===================================
-- 8. TESTE FINAL DA RPC
-- ===================================

-- Testar se a RPC funciona após limpeza
SELECT checkin_by_qr_code('PLKTK909538') as resultado_final;