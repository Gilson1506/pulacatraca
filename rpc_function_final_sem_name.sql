-- =====================================================
-- RPC FUNCTION FINAL - SEM CAMPOS INEXISTENTES
-- =====================================================
-- Erro detectado: record "v_event" has no field "name"
-- SOLUÇÃO: Usar apenas campos que EXISTEM na tabela events
    ...
    IF FOUND THEN
        RETURN json_build_object(
            ...
            'data', json_build_object(
                ...
                'ticket_type', COALESCE(v_ticket.type, v_event.ticket_type, 'Ingresso'),
...
-- ❌ REMOVIDO: v_ticket.name (possível inexistente)
-- 
-- ✅ USANDO APENAS CAMPOS EXISTENTES:
-- - v_event.title (para nome do evento)
-- - v_event.organizer_id (para organizador)
-- - v_event.created_by (fallback organizador)
-- - v_event.ticket_type (tipo de ingresso)
-- - v_ticket.type (tipo do ticket)
-- 
-- 🔧 FALLBACKS ROBUSTOS:
-- - COALESCE para campos opcionais
-- - Múltiplos fallbacks para organizer_id
-- - Valores padrão quando campos estão NULL
-- =====================================================