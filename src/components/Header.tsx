...
const Header: React.FC<HeaderProps> = ({ className = '' }) => {
  ...
  const logoUrl = isEventPage 
    ? 'https://i.postimg.cc/y8Tqr3jd/PULAKATACA-4-removebg-preview.png'
    : 'https://i.postimg.cc/YSKSHFBw/PULAKATACA-removebg-preview-1.png';
  ...
  return (
    <>
      <header className={`${isEventPage ? 'bg-transparent' : 'bg-white'} shadow-sm sticky top-0 z-50 ${className}`}>
        <div className="container mx-auto px-4">
          ...
          <div className="flex md:hidden items-center justify-between h-20 w-full">
            <div className="flex-1 flex justify-start">
              <Link to="/" className="flex items-center">
                <img
                  src={logoUrl}
                  alt="Logo PULACATRACA"
                  className="h-28 sm:h-32 w-auto cursor-pointer max-h-32 ml-0"
                  onClick={() => navigate('/')}
                />
              </Link>
            </div>
            ...
            >
              <User className={`h-7 w-7 ${isEventPage ? 'text-white' : 'text-pink-600'}`} />
            </button>
          </div>
