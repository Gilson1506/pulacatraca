...
const DashboardOverview = () => {
  ...
  const DASHBOARD_ABORT_KEY = "organizer-dashboard-overview";
  const { createNewController } = useAbortController(DASHBOARD_ABORT_KEY);
  ...
  const fetchDashboardData = async () => {
    ...
    try {
      ...
      if (error?.name === 'AbortError' || abortSignal.aborted || !isMountedRef.current) {
        console.log('⏹️ Requisição do dashboard cancelada');
        return;
      }
...
const OrganizerSupport = () => {
  ...
  const { createNewController: createSupportController } = useAbortController(SUPPORT_ABORT_KEY);
  const isSupportMountedRef = useRef(true);
...
const OrganizerEvents = () => {
  ...
  const EVENTS_ABORT_KEY = "organizer-events";
  const { createNewController: createEventsController } = useAbortController(EVENTS_ABORT_KEY);
  const isEventsMountedRef = useRef(true);
  ...
  const fetchEvents = async () => {
    ...
    try {
      ...
      // Ignorar erros de abort
      if (error?.name === 'AbortError' || abortSignal.aborted || !isEventsMountedRef.current) {
        console.log('⏹️ Requisição de eventos cancelada');
        return;
      }
