  ...
  const fetchDashboardData = async () => {
    try {
      ...
      if (eventsError) throw eventsError;
        ...
        // Buscar dados reais de ingressos vendidos
        const eventIds = eventsData.map(event => event.id);
        let totalTicketsSold = 0;
        let totalRevenue = 0;
        
        if (eventIds.length > 0) {
          // Buscar tickets vendidos para os eventos do organizador
          ...
          if (!ticketsError && ticketsData) {
            totalTicketsSold = ticketsData.length;
            // Calcular receita total baseada nos preços dos eventos
              ...
              {recentEvents.map(event => (
                <div key={event.id} className="rounded-xl border border-gray-200 overflow-hidden bg-white shadow-sm hover:shadow-md transition-shadow">
                  ...
                  <div className="p-2 md:p-3">
                    ...
                    <p className="text-xs text-gray-600 truncate mt-1">{new Date(event.start_datetime).toLocaleDateString('pt-BR')} • {event.location}</p>
          ...
          <div key={event.id} className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden hover:shadow-md transition-shadow">
            <div className="aspect-video bg-gradient-to-br from-pink-500 to-purple-600 relative">
              ...
              <div className="absolute bottom-4 left-4 text-white">
                ...
                <p className="text-sm opacity-90">{new Date(event.start_datetime).toLocaleDateString('pt-BR')} • {new Date(event.start_datetime).toLocaleTimeString('pt-BR', {hour: '2-digit', minute: '2-digit'})}</p>
              </div>
            </div>
...
const OrganizerSales = () => {
  ...
  const handleExportSalesReport = async () => {
    try {
      ...
      if (selectedEvent) {
        currentY += 10;
        doc.text(`Data: ${new Date(selectedEvent.start_datetime).toLocaleDateString('pt-BR')}`, margin, currentY);
        currentY += 10;
