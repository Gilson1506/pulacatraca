...
const OrdersSection = ({ userEmail }: { userEmail: string }) => {
      ...
      try {
        ...
        if (!ticketUsersError && ordersFromTicketUsers && ordersFromTicketUsers.length > 0) {
          ...
          const normalized = ordersFromTicketUsers.map((row: any) => {
            const status = row.tickets?.status || 'active';
            console.log('🔍 Status do ticket:', status, 'para pedido:', row.id);
            return {
              id: row.id,
              status: status,
  ...
  const getStatusText = (status: string) => {
    ...
    switch (status) {
      ...
      case 'active': return 'Concluído'; // Pago com sucesso
      case 'valid': return 'Concluído'; // Pago com sucesso
      ...
      <div className="mb-8">
        ...
        {activeTickets.length === 0 ? (
          ...
          <div className="flex flex-col gap-4">
                ...
                {/* Imagem sempre primeiro no mobile */}
                <img 
                  src={ticket.event.image || 'data:image/svg+xml;base64,'} 
      ...
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="text-center">
          ...
          <button
            ...
            className="bg-pink-900 text-white px-6 py-3 rounded-lg hover:bg-pink-800 transition-colors"
          >
            Fazer login
          </button>
        </div>
