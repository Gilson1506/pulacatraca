...
const EventStatusPage = () => {
  ...
  const fetchPedidos = async () => {
    try {
      ...
      const formattedOrders: Pedido[] = orders?.map(order => {
        console.log('🔍 Status do order:', order.status, 'para pedido:', order.id);
        return {
          id: order.id,
  ...
  const getStatusPedidoText = (status: string) => {
    console.log('🎯 Mapeando status do pedido:', status);
    switch (status) {
      ...
      case 'confirmado': return 'Concluído'; // Pago com sucesso
      default: 
        console.log('⚠️ Status do pedido não mapeado:', status);
        return 'Pendente';
    }
  };
