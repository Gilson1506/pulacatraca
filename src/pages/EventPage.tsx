...
const MapInline: React.FC<{ destinationAddress: string }> = ({ destinationAddress }) => {
  ...
  useEffect(() => {
        ...
        if (userLngLatRef.current && destLngLatRef.current) {
          ...
          if (route) {
            ...
            if (mapRef.current.loaded()) {
              ...
              const loadHandler = () => {
                addRoute();
                mapRef.current.off('load', loadHandler);
              };
              mapRef.current.on('load', loadHandler);
            }
          }
    ...
    return () => {
      isCancelled = true;
      if (mapRef.current) {
        // Remover todos os listeners antes de remover o mapa
        try {
          mapRef.current.off();
        } catch (e) {
...
const EventPage = () => {
  ...
  const { user } = useAuth(); // Usar useAuth ao invés de state local
