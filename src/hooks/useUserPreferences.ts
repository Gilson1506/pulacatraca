...
export const useUserPreferences = () => {
  ...
  const loadPreferences = async () => {
    try {
      ...
      if (localPrefs) {
        ...
        if (localPrefs.length > 50 * 1024) { // 50KB máximo
          console.warn('🧹 Preferências muito grandes, limpando...');
      ...
      if (user) {
        try {
          ...
          if (data && !error) {
            ...
            // Sincronizar com localStorage (com verificação de tamanho)
            try {
            const dataStr = JSON.stringify(serverPrefs);
            if (dataStr.length > 50 * 1024) {
              console.warn('🧹 Preferências muito grandes ao salvar');
              const essentialPrefs = {
                rememberLogin: serverPrefs.rememberLogin,
                preferredPaymentMethod: serverPrefs.preferredPaymentMethod,
                theme: serverPrefs.theme,
                language: serverPrefs.language
              };
              safeSetItem(storageKey, JSON.stringify(essentialPrefs), { fallbackToSessionStorage: true, keyDescription: storageKey });
            } else {
              safeSetItem(storageKey, dataStr, { fallbackToSessionStorage: true, keyDescription: storageKey });
            }
            } catch (error) {
  ...
  const savePreferences = async (newPreferences: Partial<UserPreferences>) => {
    try {
      ...
      // Salvar no localStorage imediatamente (com verificação de tamanho)
        try {
          const dataStr = JSON.stringify(updatedPreferences);
          if (dataStr.length > 50 * 1024) {
            console.warn('🧹 Preferências muito grandes, reduzindo...');
            const essentialPrefs = {
              rememberLogin: updatedPreferences.rememberLogin,
              preferredPaymentMethod: updatedPreferences.preferredPaymentMethod,
