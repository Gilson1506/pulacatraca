  ...
  const trackEvent = async (eventType: string, eventData: any = {}) => {
    try {
      ...
      const analyticsEvent: AnalyticsEvent = {
        ...
        // Também salvar localmente para fallback (com limite mais restrito)
        try {
        const existing = JSON.parse(localStorage.getItem('local_analytics') || '[]');
        const updated = [...existing, analyticsEvent];

        const MAX_LOCAL_EVENTS = 30;
        const MAX_LOCAL_PAYLOAD = 200 * 1024; // ~200KB

          let trimmed = updated.slice(-MAX_LOCAL_EVENTS);
          let dataStr = JSON.stringify(trimmed);

        while (dataStr.length > MAX_LOCAL_PAYLOAD && trimmed.length > 10) {
            trimmed = trimmed.slice(Math.max(trimmed.length - 10, 10));
          dataStr = JSON.stringify(trimmed);
          }

        if (!safeSetItem('local_analytics', dataStr, { fallbackToSessionStorage: true, keyDescription: 'local_analytics' })) {
          console.warn('Analytics local armazenado via fallback devido a limite de espaço.');
        }
      } catch (error) {
        console.error('Erro ao salvar analytics local:', error);
        try {
