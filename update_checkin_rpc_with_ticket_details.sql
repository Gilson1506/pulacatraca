-- ===================================================================
-- ATUALIZAÇÃO DA FUNÇÃO RPC CHECKIN_BY_QR_CODE
-- ===================================================================
-- Adiciona informações detalhadas sobre tipo de ingresso e valor real
-- ===================================================================
...
BEGIN
    ...
    -- ===================================
    -- 4. BUSCAR TIPO DE INGRESSO DETALHADO
    -- ===================================
    SELECT * INTO v_event_ticket_type 
    ...
    -- ===================================
    -- 6. VERIFICAR CHECK-IN EXISTENTE
    -- ===================================
    ...
    -- ===================================
    -- 7. REALIZAR CHECK-IN SE NECESSÁRIO
    -- ===================================
    IF v_existing_checkin.id IS NULL THEN
        ...
        RETURN json_build_object(
            'success', true,
            'rpcAction', 'NEW_CHECKIN',
            'rpcMessage', 'Check-in realizado com sucesso!',
            'ticketData', json_build_object(
                ...
                'event_location', COALESCE(v_event.location, 'Local não informado'),
                'ticket_type', COALESCE(v_event_ticket_type.name, v_ticket.ticket_type, 'Padrão'),
                'ticket_area', COALESCE(v_event_ticket_type.area, 'Área não informada'),
                'ticket_price', COALESCE(v_event_ticket_type.price, v_ticket.price, 0),
                'ticket_price_feminine', COALESCE(v_event_ticket_type.price_feminine, v_ticket.price, 0),
    ...
    ELSE
        ...
        RETURN json_build_object(
            'success', true,
            'rpcAction', 'ALREADY_CHECKED_IN',
            'rpcMessage', 'Participante já realizou check-in anteriormente',
            'ticketData', json_build_object(
                'id', v_ticket_user.id,
                'name', v_ticket_user.name,
                'email', v_ticket_user.email,
...
COMMENT ON FUNCTION checkin_by_qr_code(TEXT) IS 'Função RPC atualizada para check-in via QR code com informações detalhadas sobre tipo de ingresso e valor real.';
...
-- SELECT checkin_by_qr_code('SEU_QR_CODE_AQUI') as resultado;
