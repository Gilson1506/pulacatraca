-- ============================================
-- SISTEMA DE CHECK-IN COMPLETO - VERSÃO CORRIGIDA
-- ============================================
...
-- 1.4 Criar tabela tickets se não existir
...
-- PASSO 2: Criar tabela checkin seguindo os relacionamentos especificados
-- ============================================

-- Deletar tabela antiga se existir (para evitar conflitos)
...
CREATE TABLE IF NOT EXISTS checkin (
  ...
  -- Adicionar constraints de foreign key apenas se as tabelas existirem
...
INSERT INTO profiles (id, email, name, role, is_verified, is_active)
SELECT 
  gen_random_uuid(),
  ...
  true,
  true
WHERE NOT EXISTS (SELECT 1 FROM profiles WHERE email = 'organizador@exemplo.com');
...
DO $$
BEGIN
    RAISE NOTICE '✅ Todas as tabelas verificadas/criadas com sucesso!';
    RAISE NOTICE '✅ Tabela checkin criada com relacionamentos corretos!';
    ...
    RAISE NOTICE '📝 Execute este script no Supabase SQL Editor para criar todo o sistema!';
END $$;